...
export default function GameClient({
    ...
    return (
        <div className="flex flex-col h-[100dvh] bg-[#0A1118] overflow-hidden selection:bg-brand-blue selection:text-white font-sans text-white relative">
            ...
            <div className="p-4 sm:p-6 flex flex-col sm:flex-row gap-4 z-30 relative">
                <div className="flex-1 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-4 sm:p-5 flex items-center justify-between shadow-[0_4px_30px_rgba(0,0,0,0.2)]">
                    <div className="flex items-center gap-5">
                        <div className="transform scale-[0.7] xs:scale-[0.8] sm:scale-90 origin-left shrink-0">
                    ...
                    <div className="flex items-center gap-6 sm:gap-10">
                        <div className="flex flex-col items-center">
                            <span className="text-[8px] sm:text-[10px] font-semibold text-white/70 uppercase tracking-[0.2em] mb-1.5">Оставащи жокери</span>
                            <div className={`px-4 py-1 rounded-lg border font-bold shadow-sm text-xs sm:text-base ${team.hintsUsed >= quiz.maxHints ? 'bg-white/10 text-white/40 border-white/20' : 'bg-brand-orange/30 text-white border-brand-orange/50'}`}>
            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 pb-72 sm:pb-80 relative z-20">
                <div className="max-w-4xl mx-auto flex flex-col gap-6 sm:gap-8 pt-4">

                    {/* Question Card */}
                    <div className={`relative transition-all duration-700 pb-4 ${showSuccessAnim ? 'scale-95 opacity-0 -translate-y-8' : 'scale-100 opacity-100 translate-y-0'}`}>
                        <div className={`bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl shadow-[0_8px_32px_0_rgba(0,0,0,0.5)] overflow-hidden flex flex-col ${!showSuccessAnim ? 'animate-in fade-in zoom-in-95 duration-500' : ''}`}>

                            {/* Image Asset Container */}
                            {currentQuestion.imageUrl ? (
                                <div className="p-6 sm:p-8 bg-black/40 border-b border-white/10 flex items-center justify-center">
                                    <div className="bg-white/5 border border-white/20 rounded-2xl overflow-hidden shadow-inner p-2">
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                                        <img
                                            src={currentQuestion.imageUrl}
                                            alt="Visual challenge"
                                            className="w-full h-auto max-h-[35vh] sm:max-h-[45vh] object-contain mx-auto rounded-xl"
                                        />
                                    </div>
                                </div>
                            ) : null}
                            ...
                            {/* Question Text */}
                            <div className="p-8 sm:p-14 text-center relative overflow-hidden flex-1 flex items-center justify-center min-h-[180px] sm:min-h-[260px]">
                                ...
                                <h2 className="text-2xl xs:text-3xl sm:text-5xl font-bold leading-snug tracking-tight relative z-10 text-white">
                                    {currentQuestion.text}
                                </h2>
                            </div>

                            {/* Hint Section */}
                            <div className="px-6 pb-6 sm:px-12 sm:pb-10 flex justify-center">
                                {revealedHint ? (
                                    <div className="bg-brand-orange/20 border border-brand-orange/30 p-6 sm:p-8 rounded-2xl shadow-inner w-full max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
                                        <div className="flex items-center gap-2 text-brand-orange font-bold text-[10px] sm:text-xs uppercase tracking-wider mb-3">
                                            <Zap className="w-4 h-4" /> Жокер:
                                        </div>
                                    ...
                                    <button
                                        ...
                                        className="flex items-center justify-center w-full sm:w-auto gap-3 text-brand-orange hover:text-white bg-white/10 hover:bg-brand-orange/80 border border-brand-orange/50 px-8 py-3.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all disabled:opacity-30 disabled:grayscale group shadow-[0_4px_14px_rgba(241,146,32,0.2)] hover:shadow-[0_6px_20px_rgba(241,146,32,0.4)] hover:-translate-y-0.5 active:translate-y-0 backdrop-blur-sm"
                                    >
                                        <HelpCircle className="w-5 h-5 group-hover:rotate-12 transition-transform" />
                                        <span>{hintLoading ? "Търсене..." : "Искам жокер!"}</span>
                                    </button>
                                ) : (
                                    <div className="flex items-center gap-3 text-white/40 font-medium text-xs uppercase tracking-widest bg-white/5 px-6 py-3 rounded-full border border-white/5">
                                        <Zap className="w-4 h-4 animate-pulse" />
                                        <span>Обсъдете използването на жокер с капитана</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {me.isCaptain && team.joinRequests.length > 0 && (
                        <div className="bg-brand-orange/10 border border-brand-orange/20 rounded-3xl p-6 sm:p-8 animate-in slide-in-from-right-4 duration-500">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-xs sm:text-sm font-bold text-brand-orange uppercase tracking-[0.2em] flex items-center gap-2">
                                    <UserPlus className="w-4 h-4 sm:w-5 h-5" /> Нови заявки за присъединяване
                                </h3>
                                <span className="bg-brand-orange text-white text-[10px] font-black px-2 py-0.5 rounded-full animate-pulse">{team.joinRequests.length}</span>
                            </div>
                            <div className="space-y-3">
                                {team.joinRequests.map(r => (
                                    <div key={r.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-black/20 p-4 rounded-xl border border-white/5 gap-4 backdrop-blur-md">
                                        <span className="font-bold text-white text-base break-words w-full sm:w-auto">{r.name}</span>
                                        <div className="flex gap-2 shrink-0 w-full sm:w-auto justify-end">
                                            ...
                                            <button
                                                ...
                                                className="flex-1 sm:flex-none bg-brand-green/20 hover:bg-brand-green/30 text-brand-green border border-brand-green/30 px-6 py-2.5 rounded-lg font-bold text-xs uppercase tracking-wider transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                                            >
                                                <Check size={14} /> Приеми
                                            </button>
                                        </div>
                                    </div>
                                ))}
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 sm:p-8 bg-gradient-to-t from-[#0A1118] via-[#0A1118]/95 to-transparent pt-24 sm:pt-32 z-40">
                <div className="max-w-4xl mx-auto">
                    {errorMsg && (
                        <div className="mb-4 flex items-center justify-center gap-3 text-white font-bold bg-red-500/20 p-4 rounded-xl border border-red-500/40 text-sm sm:text-base animate-shake backdrop-blur-md">
                    ...
                    <div className="relative">
                        {showSuccessAnim ? (
                            <div className="flex items-center justify-center gap-4 bg-brand-green/30 text-white border border-brand-green/50 p-6 sm:p-8 rounded-2xl font-bold text-2xl sm:text-4xl shadow-[0_0_40px_rgba(44,150,68,0.4)] animate-in zoom-in-95 duration-300 backdrop-blur-xl">
                                <CheckCircle className="w-8 h-8 sm:w-12 sm:h-12 text-brand-green" /> ПРАВИЛНО!
                            </div>
                        ) : me.isCaptain ? (
                            <form onSubmit={handleAnswerSubmit} className="flex flex-col sm:flex-row gap-3 sm:gap-4 relative bg-white/10 p-2 sm:p-3 rounded-[2rem] border border-white/20 backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.6)]">
                                <div className="flex-1">
                                    <input
                                        ...
                                        className="w-full bg-transparent px-6 sm:px-8 py-4 sm:py-5 font-bold text-lg sm:text-2xl outline-none transition-all text-white placeholder:text-white/50"
                                        disabled={loading}
                                        spellCheck="false"
                                        autoComplete="off"
                                    />
                                ...
                                <button
                                    ...
                                    className="bg-brand-green hover:bg-green-500 text-white font-bold py-4 sm:py-5 px-8 sm:px-12 rounded-[1.5rem] shadow-[0_4px_14px_0_rgba(44,150,68,0.5)] hover:shadow-[0_6px_20px_rgba(44,150,68,0.4)] hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 flex items-center justify-center gap-3 text-lg sm:text-xl uppercase tracking-widest disabled:opacity-50 shrink-0"
                                >
                                    <span>{loading ? "..." : "ИЗПРАТИ"}</span>
                                    {!loading && <Send size={24} className="hidden sm:block" />}
                                </button>
                            </form>
                        ) : (
                            <div className="bg-white/10 backdrop-blur-2xl border border-white/20 p-6 sm:p-8 rounded-3xl flex flex-col items-center justify-center text-center shadow-[0_8px_32px_0_rgba(0,0,0,0.6)]">
                                <div className="w-12 h-12 bg-brand-blue/20 rounded-full flex items-center justify-center mb-4 animate-pulse">
                                    <MessageSquareText className="w-6 h-6 text-brand-blue" />
                                </div>
